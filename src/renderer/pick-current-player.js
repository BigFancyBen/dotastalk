module.exports = {pickCurrentPlayer};

function pickCurrentPlayer(data) {
  let allPlayers = "";
  console.log(data);
  function buildTeam(curPlayer){
    let playerString = ""
    if(curPlayer.profile){
      playerString+=`<div class="player" id="${curPlayer.profile.account_id}">
      <div class="player-name">${curPlayer.profile.personaname}</div>
      ...
      <img src="${curPlayer.profile.avatarfull}" alt="" class="player-avatar"></div>`
    }
    return playerString;
  }
  ...
  data.forEach(element => {
    console.log(element);
    allPlayers+=buildTeam(element);
  });

  let playerPickHtml = `<div class='main pick-sides pick-current'>
    <div class="team" id="pick-current-player">
      <h2 class="side">Select your profile</h2>
      ${allPlayers}
    </div>
  </div>`;
